...
function CreateGame() {
  ...
  Squares.forEach((Square, i) => {
    Square.addEventListener("click", e => {
      ...
      console.log(ArOfSquaresX[indexY], "YY");
      console.log(`click ${el} =>${IsSquare}`);
      if (!el == IsSquare) {
        --numberOfHearts;
      ...
      console.log(numberOfClicks, numberOfSquares ** 2);
      ...
      console.log(ArOfSquaresX[indexY], "ok");
    });
...
iconX.addEventListener("click", e => {
  ...
  iconHidden.style.transform = "translate(0)";
  console.log(IsSquare);
});
iconSquare.addEventListener("click", e => {
  IsSquare = true;
  console.log(IsSquare);
  iconHidden.style.transform = "translate(100%)";
});
...
//   gameArea.innerHTML += `<div class="feild x-${i % numberOfSquares +
//     1}-y-${Math.ceil(i / numberOfSquares)}"></div>`;
// }
